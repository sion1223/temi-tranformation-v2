...
# Retaining lip is on the low, lid side. A4 fits between the lips.
for name,lo,hi in [
    ('R4_low_paper_lip',(24,9,51),(25.2,315,66)),
...
for name in ('R3_fixed_electronics_deck','R3_front_wall','R3_rear_wall',
    ...
    for x in (35,218):cut(o,(x-7.4, -1, 32),(x+7.4,325,155))

# The old overhanging seals and reed switch obstructed vertical tray lift.
cut(bpy.data.objects['R3_front_top_seal'],(23,7,63),(239,12.1,79))
cut(bpy.data.objects['R3_rear_top_seal'],(23,311.9,63),(239,317,79))
...
for name,f in [('LOCKED',1),('RELEASED',13),('LID_105',265),('PICKUP',289),
               ('LIFT_35',505),('TILT_12',745),('CLOSE_REQUEST',865),('LEVEL',1105),
...
sc['revision']='R4-AUTO-LIFT / fixed left hinge / 3 motors + existing solenoid'
