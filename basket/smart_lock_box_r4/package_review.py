...
# The printable deliverable is self-contained; exclude Blender backup files.
kit=ROOT/'print_ready'
with zipfile.ZipFile(ROOT/'r4_print_ready.zip','w',zipfile.ZIP_DEFLATED,compresslevel=9) as archive:
    for p in sorted(kit.rglob('*')):
        if p.is_file() and p.suffix not in ('.blend1','.pyc'):
            archive.write(p, arcname='r4_print_ready/'+p.relative_to(kit).as_posix())
manifest=[]
